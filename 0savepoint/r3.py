...
# Pre-race vertical text objects (for displaying team info vertically)
pre_race_team_texts = {}
# =============================
...
def update(frame):
    ...
    # CORRECT PRE-RACE MODE - Shows bars like race mode + vertical text above

    # Replace the entire "if is_pre_race:" block (lines 776-821) with this:

    if is_pre_race:
        # PRE-RACE MODE: Bars with 2024 heights + logos + "0" labels + vertical text ABOVE
        ...
        for rank, team in enumerate(PRE_RACE_TEAM_ORDER):
            ...
            # 4. VERTICAL TEXT showing 2024 data (ABOVE the "0" label)
            text_parts = [f"{team}: {pts2024}"]
            if wins > 0:
            ...
            # Position vertical text ABOVE the "0" label
            text_y_start = pts_2024 + (max_pre_race_pts * 0.1)  # Start above the "0"
        ...
        # STATS BOXES - Using figure-level coordinates
        # Region 3 position: x=[48/160, 80/160], y=[0, 45/90]
        # Region 3 in pixels: x starts at 48/160 of 16-inch figure
        # We want x=69-79 and y=11-44 in FIGURE coordinates
        # But ax3 uses its own coordinate system (0-10 for x, 0-max_pts for y)
        
        # Convert figure coordinates to ax3 coordinates:
        # Figure x=69-79 is within Region 3 which spans x=48-80 in figure units
        # Region 3 ax range is 0-10 (xlim is -0.5 to 9.5, but data is 0-9)
        # Figure unit 69 = 48 + (69-48) = 48 + 21
        # In Region 3: (69-48)/(80-48) * 10 = 21/32 * 10 = 6.5625
        # Figure unit 79 = (79-48)/(80-48) * 10 = 31/32 * 10 = 9.6875
        
        # For y: Region 3 spans 0-45 in figure units, ax3 has ylim 0-max_pts
        # Figure y=11 -> 11/45 * max_pts
        # Figure y=44 -> 44/45 * max_pts
        
        stats_x_left = 5.5   # Corresponds to figure x=69
        stats_x_right = 8.75  # Corresponds to figure x=79
        stats_y_bottom_ratio = 11/45  # Corresponds to figure y=11
        stats_y_top_ratio = 44/45     # Corresponds to figure y=44
        
        stats_y_bottom = max_pts_team * stats_y_bottom_ratio
        stats_y_top = max_pts_team * stats_y_top_ratio
        
        y_offset_team = 0
        box_width_team_stat = stats_x_right - stats_x_left  # ~3.125
